...
\AddToShipoutPictureBG*{%
  \AtPageUpperLeft{%
    ...
    \raisebox{-0.5in}{%
         \makebox[0pt][r]{\textsf{SAND2026-XXXXXR}}%
}}}
...
\item Most independent sources now have analytical sensitivity functions implemented. (see issue 849 in the fixed defects table)  Previously, sensitivities with respect to independent source parameters had to rely on finite differences.  With analytical derivatives, sensitivity analysis will be more accurate and faster run times.
  \item Sensitivity analysis parameters can now be specified conveniently using the device name, alone.  Currently this new feature is only applied to independent source devices.   To use this feature, use the field \texttt{sensdevicename=string} on the \texttt{.sens} line, instead of the \texttt{param=string} field.
\end{XyceItemize}

\subsubsection*{Interface Improvements}
\begin{XyceItemize}
\item Independent PWL sources now support sensitivity analysis (see issue 849 in the fixed defects table).
\end{XyceItemize}

\subsubsection*{Important Announcements}
\begin{XyceItemize}
  \item \Xyce{} now requires CMake version 3.26 or later. This is enforced via an invocation of \\
    \emph{cmake\_minimum\_required()} within \Xyce{}'s cmake script and an error will reported 
    during configuration if this requirement is not met.
\end{XyceItemize}
