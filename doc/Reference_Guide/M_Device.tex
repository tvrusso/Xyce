...
(BSIM-SOI 4.6.1), level 70450 (BSIM-SOI 4.5.0) 
and level 70470 (BSIM-SOI 4.7.0)) MOSFET.  Please
...
{\em For the BSIM-SOI 4.x (level=70, 70450 or 70470) only):} The temperature
...
\clearpage
\subsubsection{Levels 70, 70450 and 70470 MOSFET Tables (BSIM-SOI 4.6.1, 4.5.0 and 4.7.0)}
...
Version 4.6.1 of the BSIM-SOI as the level 70 device, version 4.5.0
as level 70450 and version 4.7.0 as the level 70470.
...
Instance and model parameters of the level 70450 MOSFET are given in
tables~\ref{M_70450_Device_Instance_Params} and
\ref{M_70450_Device_Model_Params}.
Instance and model parameters of the level 70470 MOSFET are given in
tables~\ref{M_70470_Device_Instance_Params} and
\ref{M_70470_Device_Model_Params}.

The \Xyce{} BSIM-SOI models level 70, 70450 and 70470
...
Similarly, versions 4.5.0 and 4.7.0 also support internal variable outputs.
These are documented in table~\ref{M_70450_OutputVars} and table~\ref{M_70470_OutputVars}, respectively.

\textbf{NOTE:} It has been observed that the gate capacitance model of
BSIM-SOI 4.6.1 behaves differently than other versions. Specifically, the team
has seen significant disagreement of gate currents.  
This capacitance issue was introduced in version 4.6.1 and fixed in version 4.7.0.

The BSIM group did not fix the capacitance issue 
in the official version 4.6.1. However, the implementation of version 4.6.1
in several commercial simulators clearly has included a fix, based on empirical simulations.
For this reason, we have chosen to fix the issue in the \Xyce{} 
implementation of 4.6.1, even though this diverges from the official 
model published by the BSIM group.  
This model therefore matches the output from other simulators.

We are also providing BSIM-SOI 4.5.0 as the level 70450 MOSFET and the BSIM-SOI 4.7.0 as the level 70470 MOSFET.
The official BSIM-SOI 4.7.0 has a corrected capacitance model. 
It is the newest version of this model, so it would normally be assigned to level 70 
in \Xyce{}.
However, under some circumstances the \Xyce{} implementation of 4.7.0 has been 
less numerically stable than the implementation of 4.6.1.   
As such, version 4.7.0 was not set to be the level 70.
This may change in the future, if the numerical issues are corrected.

The parameters and output variables for version 4.7.0 are given in
tables~\ref{M_70470_Device_Instance_Params}, 
\ref{M_70470_Device_Model_Params} and 
\ref{M_70470_OutputVars}.

The parameters and output variables for version 4.5.0 are given in
tables~\ref{M_70450_Device_Instance_Params},
\ref{M_70450_Device_Model_Params}, and 
\ref{M_70450_OutputVars}.

Unlike BSIM-SOI 4.6.1 and 4.7.0, the 4.5.0 model's original Verilog-A source
...
\input{M_70470_Device_Instance_Params}
\input{M_70470_Device_Model_Params}
\input{M_70470_OutputVars}
