...
//-----------------------------------------------------------------------------
//
// Purpose        : Defines the DistToolDevBalanced class.  Distribution tool
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::DistToolDevBalanced
// Purpose       : ctor
// Special Notes :
// Scope         : public
// Creator       :
// Creation Date :
//-----------------------------------------------------------------------------
DistToolDevBalanced::DistToolDevBalanced(
    ...
    subcircuitParams_()
{
  ...
  devices_ = circuitContext_->getTotalDeviceCount();
  procDeviceCount_ = devices_ / numProcs_;

  procDeviceCountVec_.resize( numProcs_, 0 ); 
  procEndDeviceLines_.resize( numProcs_-1, false );
  const DeviceCountMap& deviceMap = circuitContext_->getDeviceCountMap();
  ...
  if (pdsCommPtr_->procID() == 0)
  {
    ...
    int remProc = 0;
    DeviceCountMap::const_iterator dc = deviceMap.begin();
    DeviceCountMap::const_iterator dc_end = deviceMap.end();
    procDeviceCountTable_.resize( numProcs_ );
    for ( ; dc != dc_end; dc++ )
    {
      if ((*dc).second > 0)
      {
        int perProc = (*dc).second / numProcs_;
        for (int proc=0; proc<numProcs_; proc++)
        {
          (procDeviceCountTable_[proc])[(*dc).first] = perProc; 
          procDeviceCountVec_[proc] += perProc;
        }
        int rem = (*dc).second % numProcs_;
        for (int i=0; i<rem; i++)
        {
          (procDeviceCountTable_[remProc])[(*dc).first]++;
          procDeviceCountVec_[remProc]++;
          remProc = (remProc+1) % numProcs_;
        }
/*
        std::cout << "Device " << (*dc).first << " has " << perProc << " devices per processor with " << rem << " leftover." << std::endl;
        for (int proc=0; proc<numProcs_; proc++)
        {
          std::cout << "Processor " << proc << " has " << (procDeviceCountTable_[proc])[(*dc).first] << " devices." << std::endl;
        }
*/
      }
    }
...
bool DistToolDevBalanced::sendCircuitDeviceLine(TokenVector & deviceLine )
{
  ...
  {
    // Determine device type from device line.
    ExtendedString ES1 ( " " );
    ES1 = deviceLine[0].string_;
    ES1.toUpper();

    // Check if this is a Y device
    std::string ES2;
    if ( ES1[0] == 'Y' )
    {
      ES2 = ES1.substr(1);
   
      // Check if this is a mutual inductor, linear or nonlinear.
      if (ES2 == "MIL" || ES2 == "MIN")
      {
        ES2 = "K";
      } 
    ...
    //std::cout << "Loading device " << ES1 << " ( " << ES2 << " ) " << std::endl;

    // Check if this device should be processed by currProc_.
    bool done = (((procDeviceCountTable_[currProc_])[ES2] == 0) || (procDeviceCountVec_[currProc_] == 0));
    if (done)
    {
      // Find the next processor for this device.
      //std::cout << "Processor " << currProc_ << " cannot process " << ES1 << std::endl; 
      int newProc = (currProc_+1) % numProcs_;
      while ((procDeviceCountTable_[newProc])[ES2]==0)
      {
        newProc = (newProc+1) % numProcs_;
      }
      //std::cout << "The next processor is " << newProc << " that can process " << (procDeviceCountTable_[newProc])[ES2] 
      //          << " more of device " << ES2 << std::endl; 
      

      // flush buffer and let currProc_ know that it can start processing current buffer.
      if (currProc_ != 0)
      {
        send();
        ...
        if (procDeviceCountVec_[currProc_] == 0)
        {
          // allow this proc to begin processing and then exit receive loop
          pdsCommPtr_->send( &minus2, 1, currProc_ );
          procEndDeviceLines_[ currProc_-1 ] = true;
        }
        else
        {
          // allow this proc to begin processing without exit
          pdsCommPtr_->send( &minus1, 1, currProc_ );
        }
      }

      // Switch to new processor and send processor filename, subcircuit data
      currProc_ = newProc;
    ...
    // Adjust the counts for the current device line.
    procDeviceCountVec_[currProc_]--;
    (procDeviceCountTable_[currProc_])[ES2]--;
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::endDeviceLines()
...
//-----------------------------------------------------------------------------
void DistToolDevBalanced::endDeviceLines()
{
  if (Parallel::is_parallel_run(pdsCommPtr_->comm()))
  {
    if (currProc_ > 0)
    ...
    for (int proc = 1; proc < numProcs_; proc++)
    {
      if (!procEndDeviceLines_[proc-1])
      {
        // end parsing on proc node
        pdsCommPtr_->send( &minus2, 1, proc );
        procEndDeviceLines_[proc-1] = true;
      }
    }
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::circuitStart
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolDevBalanced::circuitStart( std::string const & subcircuitName,
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::circuitEnd
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolDevBalanced::circuitEnd()
{
  if (Parallel::is_parallel_run(pdsCommPtr_->comm()))
  {
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::setFileName
...
//-----------------------------------------------------------------------------
void DistToolDevBalanced::setFileName(std::string const & fileNameIn)
{
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::distributeDevices()
...
//-----------------------------------------------------------------------------
void DistToolDevBalanced::distributeDevices()
{
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::packSubcircuitData
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolDevBalanced::packSubcircuitData( std::string const & subcircuitName,
                                           std::vector<std::string> const & nodes,
                                           std::string const & prefix,
                                           std::vector<Device::Param> const & params )
{
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::receiveCircuitData
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolDevBalanced::receiveCircuitData()
{
  if (Parallel::is_parallel_run(pdsCommPtr_->comm()))
  {
    ...
    while (true)
    {
      ...
      while (true)
      {
        ...
        if (bsize < 0)
        {
          //std::cout << "Processor " << pdsCommPtr_->procID() << " received halt request " << bsize << " with " << circuitContext_->getCurrentContextName() << " with prefix " << circuitContext_->getPrefix() << std::endl;
          // Return context to top-level context before processing device lines.
          while (circuitContext_->getPrefix() != "")
          {
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::send
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::processDeviceBuffer
...
//-----------------------------------------------------------------------------
bool
DistToolDevBalanced::processDeviceBuffer()
{
  if (Parallel::is_parallel_run(pdsCommPtr_->comm()))
  {
      ...
      while( pos < bsize )
      {
        ...
        switch( lineType )
        {
          case 'd': // process a device line
          {
            ...
            /*std::cout << "Processor " << pdsCommPtr_->procID() << " is processing line: ";
            for (unsigned int i=0; i<deviceLine.size(); i++)
              std::cout << deviceLine[i].string_ << " ";
            std::cout << std::endl;
            */
          ...
          {
            Report::DevelFatal().in("DistToolDevBalanced::processDeviceBuffer")
...
//-----------------------------------------------------------------------------
// Function      : DistToolDevBalanced::broadcastGlobalData
...
//-----------------------------------------------------------------------------
bool
DistToolDevBalanced::broadcastGlobalData()
{
  return DistToolBase::broadcastGlobalData();
}

//--------------------------------------------------------------------------
// Function      : DistToolDevBalanced::parseIncludeFile
// Purpose       : Jump to include file for 2nd pass
// Special Notes :
// Creator       : Rob Hoekstra
// Creation Date : 08/02/2004
//--------------------------------------------------------------------------
bool DistToolDevBalanced::parseIncludeFile(std::string const& includeFile,
