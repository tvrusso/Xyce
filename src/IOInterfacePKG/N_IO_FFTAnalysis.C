  ...
  for (Util::ParamList::const_iterator currentParamIt = fftBlock.begin(); currentParamIt != const_cast<Util::OptionBlock &>(fftBlock).end(); ++currentParamIt)
  {
    ...
    else if ( tag == "WINDOW")
    {
      ...
      if ( !( (windowType_ == "RECT") || (windowType_ == "RECTANGULAR") ||
              ...
              (windowType_ == "HALFCYCLESINE6") || (windowType_ == "COSINE2") || (windowType_ == "COSINE4") ) )
      {
...
bool FFTAnalysis::applyWindowFunction_()
{
  ...
  else if (windowType_ == "COSINE4")
  {
    for (int i=0; i< np_; i++)
      ftInData_[i] = sampleValues_[i]*(0.375 - 0.5*cos(2*M_PI*i/length) +  0.125*cos(4*M_PI*i/length));
  }
