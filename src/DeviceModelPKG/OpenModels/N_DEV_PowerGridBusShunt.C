...
void Traits::loadInstanceParameters(ParametricData<PowerGridBusShunt::Instance> &p)
{
  p.addPar("AT", std::string("PQP"), &PowerGridBusShunt::Instance::analysisTypeStr_)
    .setUnit(U_NONE)
    .setDescription("Analysis Type");

  p.addPar("G", 0.0, &PowerGridBusShunt::Instance::shuntConductance_)
    .setExpressionAccess(ParameterType::TIME_DEP)
    .setUnit(U_PERUNIT)
    .setDescription("Shunt Conductance");

  p.addPar("B", 0.0, &PowerGridBusShunt::Instance::shuntSusceptance_)
    .setExpressionAccess(ParameterType::TIME_DEP)
    .setUnit(U_PERUNIT)
    .setDescription("Shunt Susceptance");
}

void Traits::loadModelParameters(ParametricData<PowerGridBusShunt::Model> &p)
{}
...
bool Instance::processParams ()
{
  ...
  if ( !given("G") && !given("B") )
  {
    UserError(*this) << "Either G or B must be specified for PowerGridBusShunt device.";
  }
  else if ( (shuntConductance_ == 0.0) && (shuntSusceptance_ == 0.0) )
  {
    UserError(*this) << "Either G or B must be non-zero for PowerGridBusShunt device.";
  }

  // Calculate the Y parameters from the shunt data
  std::complex<double> yVal = std::complex<double>(shuntConductance_,shuntSusceptance_);
 
  // get the real and imaginary parts.  It makes the rest of the code (in the F and dF/dX
  // functions) more readable
  g11 = shuntConductance_;
  b11 = shuntSusceptance_;
  g12 = -1.0*shuntConductance_;
  b12 = -1.0*shuntSusceptance_;
  g21 = g12;
  b21 = b12;
  g22 = g11;
  b22 = b11;
                             ...
                             const std::vector<int> & extLIDVecRef )
{
  ...
  // in general, there will be 4 I/0 nodes for each shunt.
...
{
  if (deviceMap.empty() || (deviceMap.find("POWERGRIDBUSSHUNT")!=deviceMap.end())
                        || (deviceMap.find("PGBS")!=deviceMap.end()))
  {
    Config<Traits>::addConfiguration()
      .registerDevice("PowerGridBusShunt", 1)
      .registerDevice("PGBS", 1)
      .registerModelType("PowerGridBusShunt", 1);
  }
}
