...
/* Deallocate the vectors that are located in the matrix frame. */
    ...
    FREE (Matrix->Pivots );
    FREE (Matrix->row_scale_factors );
    FREE (Matrix->col_scale_factors );
    FREE (Matrix->MyStuff->ColStart_s );
    FREE (Matrix->MyStuff->ColDiag );
    FREE (Matrix->MyStuff->MyD );
    FREE (Matrix->MyStuff->Dest );
    FREE (Matrix->MyStuff->ind_list );
    FREE (Matrix->MyStuff->ind_list_i );
    FREE (Matrix->MyStuff );
 
    FREE( num_returned_elements );
    FREE( returned_elements ); 
