...
int PCEDirectSolver::doSolve( bool reuse_factors, bool transpose )
{
  ...
  // Generate the block PCE Jacobian.
  formPCEJacobian();
...
void PCEDirectSolver::createBlockStructures()
{
  ...
  else if (solver_ == "BLOCK_BASKER") 
  {
    ...
    else
    {
      ...
      // Note, ERK: for PCE this step probably isn't needed.
      for (int i=0; i < n_; i++)
      {
...
void PCEDirectSolver::formPCEJacobian()
{
  ...
  if ( solver_ == "LAPACK" )
  {
    ...
    std::cout << densePCEJacobian_.denseMtx;
    std::cout << std::endl;
  }
...
int PCEDirectSolver::numericFactorization()
{
  ...
  if ( (builder_.getPDSComm())->procID() == 0 )
  {
    if ( solver_ == "LAPACK" )
    {
      lapackSolver_->setMatrix( Teuchos::rcp( &(densePCEJacobian_.denseMtx), false ) );
...
void PCEDirectSolver::printPCEJacobian( const std::string& fileName )
{
  ...
  if (myProc == 0)
  {
    if (solver_ == "LAPACK")
    {
      ...
      // separate entries)).
      out << densePCEJacobian_.rows << " " << densePCEJacobian_.cols << std::endl;

      // Set precision.
      out.precision( 16 );
      out << std::scientific;

      for (int j=0; j<densePCEJacobian_.cols; j++)
      {
        for (int i=0; i<densePCEJacobian_.rows; i++)
        {
          out << densePCEJacobian_.denseMtx( i, j ) << std::endl;
        }
      }
    }
